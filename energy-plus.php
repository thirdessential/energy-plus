...
* @wordpress-plugin
* Plugin Name:       Energy+
* Plugin URI:        https://en.er.gy/docs/plus?v=1.2.4
...
*/
	...
	if (!defined( 'EnergyPlus_Version' )) {
		define( 'EnergyPlus_Version',   '1.2.4' );
	}
	...
	*/
	spl_autoload_register(function ($class) {
		...
		if (stripos($me, 'energyplus') === FALSE && stripos($me, 'reactors__') === FALSE && stripos($me, 'widgets__') === FALSE) {
			return;
		}
...
/*	if (get_option('energyplus_active')) {
		require EnergyPlus_Framework . 'libs/puc/plugin-update-checker.php';
		$myUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
			sprintf('https://en.er.gy/api/v1/update?c=%s&u=%s&p=1', get_option('energyplus_active'), esc_url_raw(get_bloginfo('url'))),
			__FILE__,
			'energy-plus'
		);
	}
