...
class EnergyPlus_Products extends EnergyPlus {
	...
	*/
	...
	public static function filter($filter = false) {
		...
		if (!$filter) {
			$filter['post_status'] = array('publish', 'private', 'pending');
			$filter['offset']      = 0;
			$filter['page']        = 1;
			$filter['q']           = '';
			$filter['orderby']     = "date";
		...
		if ('pending' === EnergyPlus_Helpers::get('status', '')) {
						...
						$critical_stocks_ids = $wpdb->get_results(
							...
							$outofstocks_ids = $wpdb->get_results(
								...
								$onsales_ids = $wpdb->get_results(
									...
									if (EnergyPlus_Helpers::post('q', '') OR EnergyPlus_Helpers::get('s', '')) {
										add_filter( 'posts_where', 'EnergyPlus_Products::extend_wp_query_where', 10, 2 );
									}
									...
									if (EnergyPlus_Helpers::post('q', '') OR EnergyPlus_Helpers::get('s', '')) {
										remove_filter( 'posts_where', 'EnergyPlus_Products::extend_wp_query_where');
									}
							...
							*/
							...
							public static function extend_wp_query_where( $where, $wp_query ) {
								...
								if (EnergyPlus_Helpers::post('q', '')) {
									$energy_q = "%".esc_sql(sanitize_text_field(wc_clean(EnergyPlus_Helpers::post('q', ''))))."%";
								}

								if (EnergyPlus_Helpers::get('s', '')) {
									$energy_q = "%".esc_sql(sanitize_text_field(wc_clean(EnergyPlus_Helpers::get('s', ''))))."%";
								}
