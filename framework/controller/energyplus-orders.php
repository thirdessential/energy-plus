...
class EnergyPlus_Orders extends EnergyPlus {
	...
	*/
	...
	public static function index($filter = array()) {
		...
		if(isset($_REQUEST['filterBy']) && (strpos($_REQUEST['filterBy'], 'pickup_time') !== false ) ){
				$list['statuses_count']["count"] = self::get_orders( $filter['post_status'], $filter,  $filter['page'])['count'];
		...
		switch ( $mode = ( !empty($filter['mode']) ? absint($filter['mode']) : EnergyPlus::option('mode-energyplus-orders', 1) ) )	{
			...
			case 98:
			if(isset($_REQUEST['filterBy']) && (strpos($_REQUEST['filterBy'], 'pickup_time') !== false ) ){
				$orders['orders'] = self::_group_by($orders['orders'], 'created_at','pickup');
			}else{
				$orders['orders'] = self::_group_by($orders['orders'], 'created_at');
			}
			//file_put_contents(plugin_dir_path( __FILE__ )."f1.txt",  print_r(array($orders['orders']), true), FILE_APPEND | LOCK_EX );
	...
	*/
	...
	private static function _group_by($array, $key,$p='') {
		...
		if(!empty($p)){
			foreach($array as $val) {
				...
				$datacreated = get_post_meta($val['id'],'pickup_date_time', true);
				if($datacreated >= strtotime(date_i18n('Y-m-d H:i'))){
					$return['d221']['title']    = 'Recent order';
					$return['d221']['orders'][] = $val;
				}else{
					$orders_passed[] =  $val;
				}
			...
			if(!empty($orders_passed)){
				foreach ($orders_passed as $ord) {
					$return['d222']['title']    = 'passed order';
					$return['d222']['orders'][] = $ord;
				}
			...
			if (!empty($filter['search']) && (2 < strlen($filter['search']) OR 0 ===  strlen($filter['search']))) {
				...
				if ( $the_query->have_posts() ) {
				    ...
				    while ( $the_query->have_posts() ) {
				        $the_query->the_post();
				        if( in_array(get_the_id(), $results)){
				...
				// if (!empty($filter['search']) && ( strlen($filter['search']) > 0 ) )
				// {
				...
				if(isset($_REQUEST['filterBy']) && (strpos($_REQUEST['filterBy'], 'pickup_time') !== false ) ){
					$query_args['meta_query'] = array(
					        ...
					        array(
					            ...
					            'value' => strtotime(date_i18n('Y-m-d')),
					            'compare' => '>=',
					            'type' => 'NUMERIC'
					        ),
					        // array(
				...
				if(isset($_REQUEST['filterBy']) && (strpos($_REQUEST['filterBy'], 'pickup_time') !== false ) ){
					$query_args['meta_key'] = 'pickup_date_time';
